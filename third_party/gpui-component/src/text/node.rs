...
use crate::{
    ...
    text::{
        CodeBlockActionsFn, SourceHighlight,
...
impl CodeBlock {
    ...
    pub(crate) fn new(
        ...
        source_highlights: Vec<(Range<usize>, HighlightStyle)>,
    ) -> Self {
        let mut styles = vec![];
        ...
        styles = gpui::combine_highlights(styles, source_highlights).collect();
...
pub(crate) struct NodeContext {
    ...
    pub(crate) source_highlights: Vec<SourceHighlight>,
}
        ...
        for inline_node in children {
            ...
            if let Some(image) = &inline_node.image {
                ...
                for (range, style) in &inline_node.marks {
                    ...
                    let mut highlight = style.highlight.unwrap_or_default();
                    if style.bold {
