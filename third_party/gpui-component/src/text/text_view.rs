...
/// A style applied to one byte range in the original Markdown source.
...
enum Update {
    ...
    SourceHighlights(Vec<SourceHighlight>),
}
...
struct UpdateFuture {
    ...
    current_source_highlights: Vec<SourceHighlight>,
    timer: Timer,
...
impl UpdateFuture {
    ...
    fn new(
        ...
        style: TextViewStyle,
        text: SharedString,
        source_highlights: Vec<SourceHighlight>,
        highlight_theme: Arc<HighlightTheme>,
        ...
        Self {
            ...
            current_style: style,
            current_text: text,
            current_source_highlights: source_highlights,
            timer: Timer::never(),
            rx: Box::pin(rx),
            tx_result,
            delay,
            ...
            match self.rx.poll_next(cx) {
                Poll::Ready(Some(update)) => {
                    let parse_delay = match update {
                        ...
                        Update::SourceHighlights(highlights)
                            if self.current_source_highlights != highlights =>
                        {
                            self.current_source_highlights = highlights;
                            Some(Duration::ZERO)
                        }
                        _ => None,
                    };
                    if let Some(delay) = parse_delay {
            ...
            match self.timer.poll_next(cx) {
                Poll::Ready(Some(_)) => {
                    let res = parse_content(
                        ...
                        &self.current_source_highlights,
                    );
...
impl TextView {
    ...
    /// Apply styles to byte ranges in the original Markdown source.
    pub fn source_highlights(
        mut self,
        highlights: impl IntoIterator<Item = SourceHighlight>,
    ) -> Self {
        let mut highlights = highlights.into_iter().collect::<Vec<_>>();
        highlights.retain(|highlight| {
            highlight.range.start < highlight.range.end
                && highlight.range.end <= self.raw.len()
                && self.raw.is_char_boundary(highlight.range.start)
                && self.raw.is_char_boundary(highlight.range.end)
        });
        ...
        if let Some(init_state) = &mut self.init_state {
            match init_state {
                ...
                InitState::Initialized { tx } => {
                    let _ = tx.try_send(Update::SourceHighlights(highlights));
                }
            }
        }
        self
...
fn parse_content(
    ...
    let mut node_cx = NodeContext {
        ...
        source: (!source_highlights.is_empty()).then(|| text.to_owned().into()),
        source_highlights: source_highlights.to_vec(),
        ..NodeContext::default()
    };
    ...
    node_cx.source = None;
    node_cx.source_highlights.clear();
