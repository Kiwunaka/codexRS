...
> [!IMPORTANT]
> 当前代码树以 `v0.1.0-rc.3` 为目标。Windows 已通过针对精确 stable
...
页面下载。RC3 文件为 `codexrs-v0.1.0-rc.3-windows-x86_64.zip` 和
`codexrs-v0.1.0-rc.3-linux-x86_64.tar.gz`，并附有 `SHA256SUMS.txt`。
解压前请校验 checksum：Linux 可运行
`grep ' \./codexrs-v0.1.0-rc.3-linux-x86_64.tar.gz$' SHA256SUMS.txt | sha256sum -c -`；Windows
可将 `(Get-FileHash .\codexrs-v0.1.0-rc.3-windows-x86_64.zip -Algorithm SHA256).Hash`
...
Ubuntu CI 会构建并测试二进制文件，但解压后的归档尚未完成 desktop smoke 测试。RC3
