...
mod windows {
    ...
    impl WindowsBackgroundCompletionNotifier {
        pub(super) fn new() -> Option<Self> {
            ...
            let thread = thread::Builder::new()
                ...
                .spawn(move || run_host(command_receiver, ready_sender, host_shutdown))
                .ok()?;
    ...
    fn run_host(commands: Receiver<()>, ready: Sender<w::HWND>, shutdown: Arc<AtomicBool>) {
        ...
        window.on().wm(w::co::WM::APP, move |_| {
            let Some(hwnd) = wake_window.hwnd().GetAncestor(w::co::GA::ROOT) else {
                return Ok(Some(0));
            };
            ...
            if commands.try_recv().is_ok() && wake_registered.load(Ordering::Acquire) {
    ...
    fn add_notification_icon(hwnd: w::HWND) -> Result<(), w::co::ERROR> {
        ...
        data.hIcon = icon;
        data.set_szTip(NOTIFICATION_TITLE);
    ...
    #[cfg(test)]
    mod tests {
        use super::{NOTIFICATION_BODY, NOTIFICATION_TITLE};
